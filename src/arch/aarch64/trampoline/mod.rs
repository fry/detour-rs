...
impl Builder {
  ...
  unsafe fn next_instruction(&mut self) -> Result<Instruction> {
    ...
    let instruction = std::ptr::read(instruction_address);
    // Disassemble the next instruction
    match bad64::decode(instruction, instruction_address) {
      None => Err(Error::InvalidCode)?,
      Some(instruction) => {
        // Keep track of the total amount of bytes
        self.total_bytes_disassembled += 4;
        Ok(instruction)
      },
    }
  }
